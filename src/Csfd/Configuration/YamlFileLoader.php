 ...
 *
 * Unlinke original YamlFileLoader this one supports nested definitions
 *
...
class YamlFileLoader extends FileLoader
{
    ...
    private function parseDefinitions($content, $file)
    {
        ...
        $this->parseDefinitionNode($content['services'], $file);
    }

    private function parseDefinitionNode($node, $file, $namespace = [])
    {
    	foreach ($node as $id => $children) {
    		$ns = $namespace; // keep original untouched
    		array_push($ns, $id);
        	if (!isset($children['class'])) {
        		// assume nested node
        		$this->parseDefinitionNode($children, $file, $ns);
        	} else {
        		$this->parseDefinition(implode('.', $ns) , $children, $file);
        	}
        }
    }
